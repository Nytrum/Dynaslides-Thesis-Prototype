...
### Backend
The backend is responsible for translating the powerpoint file and send it to the frontend. It can be used through a command-line interface with the need of deploying the frontend application.
The most important files and directories are:
 + *config* - This is where properties of the dynaslides application can be changed;
 + *js* - This is where all Javascript files, including the plugins, are located;
 + *app.js* - This is where the backend main implementation is done; It uses express.js to create an endpoint that provides the translation to the frontend implementation;
...
### ConversionTests
The conversion tests are a way to see if the elements of the original presentation are equal or similar to the elements on the new presentation format.
### Frontend
The frontend is responsible for providing an user interface capable of sending and downloading requests made to the backend implementation. 
The most important files and directories are:
 + *src* - This is where all the implementation is present. The frontend takes advantage of the React Javascript Framework to display the user interface and send the request to the backend;
 + *src/App.js* - This is where the magic happens. All the functions to communicate with backend application are here. The frontend application has the "http://localhost:7000/getDynaslidesPresentation" endpoint and this must to be changed to the actual endpoint of the backend in order to work.;

### Server
The server is an express.js implementation which sets up a socket.io server that will do all the communication between the master presentation and the client presentation. It has to be place in a public endpoint and this endpoint must be specified in the config file of the dynaslides application. 
## Installation and usage
...
### Website version:
To setup the website environment locally, the presenter first has to do the following:
